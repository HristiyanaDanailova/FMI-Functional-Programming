...
;instead of the division by 3, applied in the day1-task, here the modulo operation is used. The modulus is the multiplication of all num values for every monkey from the 'test'
;checks if an element (after an operation has been executed) is divisible by num
;the following pair is returned, based on the result of the division : (element % modTot, #t/#f position)
...
;adds the given element to the list of elements of monkey number-'i' of 
...
;finds the total modulus number
(define (modulTotal l)
  (foldr * 1(map (lambda(x) (car (car (cdr (cdr x))))) l)))

;goes through 10000 rounds
...
(display (calc (10000-rounds inp6 (modulTotal inp6))))
(close-input-port in)